...
class BARDInteractionGraph(InteractionGraph):
    ...
    def create_user_MultiADI_splits(self):
        ...
        for user_id, attributes in self.user_data.items():
            ...
            data[key_value].append(user_id)

        group_mapping = {
            "Dialectal": ["Bahrain", "Iraq", "Kuwait", "Oman", "Qatar", "Saudi_Arabia", "UAE", "Yemen", "Jordan", "Lebanon", "Palestine", "Syria", "Libya", "Morocco", "Tunisia", "Algeria", "Egypt", "Sudan"],
            "MSA": ["MSA"]
        }

        self.range_user_ids = {}
        for group, keys in group_mapping.items():
            self.range_user_ids[group] = []
            for key in keys:
                self.range_user_ids[group].extend(data.get(key, []))  # Extend list if key exists
